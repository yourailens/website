...
import { LIGHTING_TYPE_LABELS, LIGHTING_MOOD_LABELS } from "@/data/lighting_presets";
...
export default function LightingPresetDetailExperience({ item }: { item: LightingPreset }) {
  ...
  async function handleDownload() {
    setDownloading(true);
    try {
      await fetch(`/api/lighting-presets/${item.slug}`, { method: "POST" });
    } catch {}
    try {
  ...
  return (
    <IndustryShell>
      ...
      <IndustryTopBar>
        <IndustryBreadcrumb
          items={[
            { label: "LIGHTING", href: "/lighting-presets" },
        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-14">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap gap-2">
              <span className={pill}>{LIGHTING_TYPE_LABELS[item.lighting_type]}</span>
              <span className={pill}>{LIGHTING_MOOD_LABELS[item.mood]}</span>
              {item.color_temp && <span className={pill}>{item.color_temp}</span>}
              {item.featured && (
              ...
              <div className="mt-8">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {item.style_tags.map((t) => (
                    <Link
                      key={t}
                      href={`/lighting-presets?q=${encodeURIComponent(t)}`}
