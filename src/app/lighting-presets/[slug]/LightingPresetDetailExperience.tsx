...
import { LIGHTING_TYPE_ACCENTS, LIGHTING_TYPE_LABELS, LIGHTING_MOOD_LABELS } from "@/data/lighting_presets";

export default function LightingPresetDetailExperience({ item }: { item: LightingPreset }) {
  ...
  async function handleDownload() {
    setDownloading(true);
    try { await fetch(`/api/lighting-presets/${item.slug}`, { method: "POST" }); } catch {}
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-14">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-bold ${accent}`}>{LIGHTING_TYPE_LABELS[item.lighting_type]}</span>
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600">{LIGHTING_MOOD_LABELS[item.mood]}</span>
                {item.color_temp && <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600">{item.color_temp}</span>}
              ...
              {item.style_tags.length > 0 && (
                <div className="mt-8"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Style Tags</p>
                  <div className="mt-3 flex flex-wrap gap-2">{item.style_tags.map((t) => <Link key={t} href={`/lighting-presets?q=${encodeURIComponent(t)}`} className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600 transition hover:border-blue-200 hover:text-blue-700">#{t}</Link>)}</div>
                </div>
              )}
            </div>
          </div>
