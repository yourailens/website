...
import { AESTHETIC_STYLE_LABELS, AESTHETIC_ERA_LABELS } from "@/data/mood_boards";
...
export default function MoodBoardDetailExperience({ item }: { item: MoodBoard }) {
  ...
  async function handleDownload() {
    setDownloading(true);
    try {
      await fetch(`/api/mood-boards/${item.slug}`, { method: "POST" });
    } catch {}
    try {
        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-14">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap gap-2">
              <span className={pill}>{AESTHETIC_STYLE_LABELS[item.aesthetic]}</span>
              <span className={pill}>{AESTHETIC_ERA_LABELS[item.era]}</span>
              {item.featured && (
              ...
              <div className="mt-8">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {item.style_tags.map((t) => (
                    <Link
                      key={t}
                      href={`/mood-boards?q=${encodeURIComponent(t)}`}
