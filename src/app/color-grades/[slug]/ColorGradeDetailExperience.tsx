...
import { COLOR_GRADE_STYLE_LABELS, COLOR_GRADE_MOOD_LABELS } from "@/data/color_grades";
...
export default function ColorGradeDetailExperience({ item }: { item: ColorGrade }) {
  ...
  async function handleDownload() {
    setDownloading(true);
    try {
      await fetch(`/api/color-grades/${item.slug}`, { method: "POST" });
    } catch {}
    try {
  ...
  return (
    <IndustryShell>
      ...
      <IndustryTopBar>
        <IndustryBreadcrumb
          items={[
            { label: "COLOR GRADES", href: "/color-grades" },
        ...
        <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-14">
          ...
          >
            ...
            {item.dominant_colors.length > 0 && (
              <div className="mt-3 flex overflow-hidden border border-white/12">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap gap-2">
              <span className={pill}>{COLOR_GRADE_STYLE_LABELS[item.grade_style]}</span>
              <span className={pill}>{COLOR_GRADE_MOOD_LABELS[item.mood]}</span>
              {item.featured && (
              ...
              <div className="mt-8">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {item.style_tags.map((t) => (
                    <Link
                      key={t}
                      href={`/color-grades?q=${encodeURIComponent(t)}`}
