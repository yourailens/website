...
import { COLOR_GRADE_STYLE_ACCENTS, COLOR_GRADE_STYLE_LABELS, COLOR_GRADE_MOOD_LABELS } from "@/data/color_grades";

export default function ColorGradeDetailExperience({ item }: { item: ColorGrade }) {
  ...
  async function handleDownload() {
    setDownloading(true);
    try { await fetch(`/api/color-grades/${item.slug}`, { method: "POST" }); } catch {}
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50">
        <div className="border-b border-slate-200 bg-white"><div className="w-[95%] mx-auto py-3 flex items-center gap-4">
          <Link href="/color-grades" className="flex shrink-0 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-600 shadow-sm transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2"><path d="M19 12H5M12 5l-7 7 7 7"/></svg>Color Grades</Link>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-14">
            <div className={`w-full shrink-0 lg:sticky lg:top-28 ${isPortrait ? "lg:max-w-sm" : isSquare ? "lg:max-w-md" : "lg:max-w-2xl"}`}>
              ...
              {item.dominant_colors.length > 0 && (
                <div className="mt-3 flex overflow-hidden rounded-2xl border border-slate-200">
                  {item.dominant_colors.map((c, i) => <div key={i} className="flex-1 h-8" style={{ backgroundColor: c }} title={c} />)}
                </div>
              )}
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap gap-2">
                <span className={`rounded-full px-3 py-1 text-xs font-bold ${accent}`}>{COLOR_GRADE_STYLE_LABELS[item.grade_style]}</span>
                <span className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600">{COLOR_GRADE_MOOD_LABELS[item.mood]}</span>
              ...
              {item.style_tags.length > 0 && <div className="mt-8"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Style Tags</p><div className="mt-3 flex flex-wrap gap-2">{item.style_tags.map((t) => <Link key={t} href={`/color-grades?q=${encodeURIComponent(t)}`} className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-600 transition hover:border-blue-200 hover:text-blue-700">#{t}</Link>)}</div></div>}
            </div>
          </div>
        </div>
      </div>
