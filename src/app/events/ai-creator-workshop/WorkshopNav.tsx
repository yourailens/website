...
export default function WorkshopNav({
  ...
  const statusLabel = registrationClosed
    ...
    : soldOut
      ? "Cohort full"
      : `~${WORKSHOP_SEATS_LEFT_DISPLAY} spots open`;
  ...
  return (
    ...
    >
      ...
      {registrationClosed ? (
        <span
          className="cursor-not-allowed rounded-full border-2 border-slate-200 bg-slate-100 px-5 py-2.5 text-sm font-bold uppercase tracking-wide text-slate-400"
          aria-disabled="true"
        >
          Registration closed
        </span>
