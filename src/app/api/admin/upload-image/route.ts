...
async function insertImageRow(
  ...
  const aspectVal =
    aspect === "square" || aspect === "portrait" || aspect === "landscape" ? aspect : null;
...
export async function POST(request: Request) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    title = String(form.get("title") ?? "").trim() || "Image";
    ...
    try {
      ...
      const key = `gallery/images/${Date.now()}-${safeObjectFilename(file.name)}`;
